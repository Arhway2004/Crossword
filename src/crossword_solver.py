        ...
        for row in range(self.rows):
            for col in range(self.cols):
                ...
                if self.grid[row][col] == "X":
                    ...
                    self.pen.color(self.colors["blocked"])  # Same color for border and fill
                    self.pen.begin_fill()
                    for _ in range(4):
...
def main():
    ...
    if not nogui:
        try:
            ...
            drawer.draw_grid()
            drawer.slots_storage = slots  # Store slots for numbering
            drawer.draw_slot_numbers(slots)  # Draw slot numbers initially
