...
class CrosswordExtractor:
    ...
    def _load_image(self):
        ...
        print(
            f"Image loaded successfully: {self.image.shape[1]}x{self.image.shape[0]} pixels"
        )
    ...
    def _detect_grid_lines_morphology(
    ...
    def _estimate_grid_dimensions(
        self, h_lines: List[int], v_lines: List[int], w: int, h: int
    ...
    def _extract_grid_layout(
        self, grid_region: Tuple[int, int, int, int]
    ) -> List[List[str]]:
        """
        ...
        binary = cv2.adaptiveThreshold(
            blur, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY_INV, 15, 3
        )
        ...
        if filled_ratio > 0.6:
            print(
                f"Warning: {filled_ratio*100:.1f}% cells are filled. Inverting grid..."
            )
